...
// --- Advanced Particles: Golden Sparks & Paper Butterflies ---
...
export default function QuantumOrigamiButton() {
  ...
  // 0: Idle
  // 1: Melt to paper
  // 2: Origami folds & 3D rotation
  // 3: Product teleport & weight drop
  // 4: Ribbon wraps & Gold Seal
  // 5: Success & Butterflies
  const handleClick = () => {
    if (step !== 0) return;
    mouseX.set(0);
    mouseY.set(0);
  ...
  return (
    <div className="relative flex items-center justify-center min-h-[600px] w-full bg-[#FAFAFA] font-sans overflow-hidden">
      ...
      >
        ...
        >
          {/* Animated Specular Reflection (Glass highlight) */}
          <motion.div 
          ...
          {/* Animated Gradient Border (Idle State) */}
          <AnimatePresence>
            {step === 0 && (
              <motion.div 
          ...
          <div className="relative flex items-center justify-center w-full h-full z-10">
            ...
            >
              {/* Product Teleport & Weight Bounce Container */}
              <motion.div
                animate={{ 
                  y: step === 3 ? [0, 15, -5, 0] : 0, 
              ...
              >
                <svg viewBox="0 0 120 120" className="w-full h-full overflow-visible drop-shadow-xl">
                  ...
                  <motion.path
                    ...
                    fill="none"
                    stroke={step === 5 ? "#22C55E" : "#111"}
                    strokeWidth="4"
                    strokeLinecap="round"
                    initial={false}
                    animate={{ 
                      pathLength: step >= 1 && step <= 4 ? 0 : 1,
                      opacity: step >= 1 && step <= 4 ? 0 : 1,
                      y: step === 3 ? 5 : 0 // Handle flex during drop
                    }}
                  ...
                  {/* 3. TELEPORTING PRODUCT CUBE */}
                  <motion.rect
                    x="45" y="-30" width="30" height="30" rx="4"
                    fill="url(#cubeGradient)"
                    initial={{ opacity: 0, y: -60, scale: 0 }}
                    animate={{ 
                      opacity: step === 3 ? [0, 1, 1, 0] : 0,
                      y: step === 3 ? [-60, 25] : -60,
                      scale: step === 3 ? [0, 1.2, 1, 0.8] : 0,
                      rotate: step === 3 ? [0, 180] : 0
                    }}
                  ...
                  {/* 4. ORIGAMI PAPER / BAG BODY */}
                  <motion.path
                    initial={false}
                    animate={{
                      d: step === 1 
                        ? "M 10 10 L 110 10 L 110 110 L 10 110 Z" // Flat Paper
                        : "M 30 45 L 90 45 L 85 105 C 85 110 80 115 60 115 C 40 115 35 110 35 105 Z", // Bag Body
                  ...
                  {/* 6. CLOSING FLAP (Luxury Packaging) */}
                  <motion.path
                ...
                {/* LUXURY GOLDEN SPARKS (At Seal Impact) */}
            ...
            {/* PAPER BUTTERFLIES (Success Effect) */}
