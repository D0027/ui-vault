            ...
            >
              ...
              <div className="flex flex-col gap-5">
                <div className="flex flex-col gap-1.5">
                   ...
                   <input type="email" placeholder="name@example.com" className="w-full px-5 py-3.5 rounded-xl bg-black/50 border border-neutral-800 text-white placeholder-neutral-700 focus:outline-none focus:border-amber-500/50 focus:ring-1 focus:ring-amber-500/50 transition-all" />
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                   ...
                   <input type="password" placeholder="••••••••" className="w-full px-5 py-3.5 rounded-xl bg-black/50 border border-neutral-800 text-white placeholder-neutral-700 focus:outline-none focus:border-amber-500/50 focus:ring-1 focus:ring-amber-500/50 transition-all" />
                </div>
            ...
            >
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                <motion.div variants={itemVariants} className="relative group">
                  ...
                  <input type="email" placeholder="Email Address" required className="w-full bg-black/40 border border-white/5 rounded-xl py-3.5 pl-11 pr-4 text-white text-sm placeholder-[#A1A1AA] focus:outline-none focus:border-white/20 focus:bg-black/60 transition-all shadow-inner" />
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="relative group">
                  ...
                  <input type={showPassword ? "text" : "password"} placeholder="Password" required className="w-full bg-black/40 border border-white/5 rounded-xl py-3.5 pl-11 pr-11 text-white text-sm placeholder-[#A1A1AA] focus:outline-none focus:border-white/20 focus:bg-black/60 transition-all shadow-inner" />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-4 flex items-center text-[#A1A1AA] hover:text-white transition-colors focus:outline-none">
                    {showPassword ? (
                ...
                <motion.div variants={itemVariants} className="pt-2">
                  <motion.button whileHover={{ scale: status === "idle" ? 1.02 : 1 }} whileTap={{ scale: status === "idle" ? 0.98 : 1 }} type="submit" className="relative w-full h-[50px] bg-gradient-to-r from-[#7C3AED] to-[#A855F7] text-white font-semibold rounded-xl shadow-[0_0_20px_rgba(168,85,247,0.3)] flex items-center justify-center overflow-hidden transition-shadow hover:shadow-[0_0_25px_rgba(168,85,247,0.5)]">
                    <AnimatePresence mode="wait">
                      {status === "idle" && <motion.span key="idle" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>Sign In</motion.span>}
                      {status === "loading" && <motion.span key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}><svg className="animate-spin h-5 w-5 text-white" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></motion.span>}
                      {status === "success" && <motion.span key="success" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} transition={{ type: "spring", stiffness: 200, damping: 10 }}><svg className="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg></motion.span>}
                    </AnimatePresence>
                ...
                <motion.div variants={itemVariants} className="text-center pt-2">
                  <p className="text-xs text-[#A1A1AA] font-medium">Don't have an account? <button type="button" className="text-white hover:text-[#A855F7] font-semibold transition-colors">Sign up</button></p>
                </motion.div>
